...
const outputPath = path.join(outputDir, "신계약가이드라인_공통코어_샘플결과_v2.xlsx");
...
const ruleRows = [
  ...
  ["R-003", "시행예정", "소액암진단", "", "LI00113", 1000, 2000, "", 1000, 2000, "", 3000, "", "N-002,N-003", "", "Sheet1!B7:J7", "소액암진단 단일건 한도 상향 초안. 인별합산 및 주석 영향 검토 필요", "초안"],
...
const noteRows = [
  ...
  ["N-002", "암진단-소액암 연계", "암진단 가입시 소액암 진단 가입필수", "연계조건", "R-002,R-003", 1, "Sheet1!B19", "소액암진단 단일건 한도 상향 시 연계조건 영향 여부 검토 가능"],
  ["N-003", "소액암 인별합산 예외", "소액암 진단 인별합산 한도 5천만원 (단, 66세 이상 3천만)", "예외/상향기준", "R-003", 1, "Sheet1!B20", "단일건 2000 상향 후에도 인별합산 3000 및 예외 주석 유지 여부 검토 필요"],
...
const mapRows = [
  ...
  ["R-003", "N-002", "단일행", "연계조건 적용", "Sheet1!E7 + Sheet1!B19", "Y"],
...
const changeRows = [
  ["C-001", "R-003", "현행 -> 시행예정", "일반/건강 단일건 1000 / 간편 단일건 1000", "일반/건강 단일건 2000 / 간편 단일건 2000", "소액암진단 단일건 한도 상향", "검토 필요", "인별합산 및 주석 N-002, N-003 영향 검토 필요"],
];
...
ruleMaster.getRange("A1:R1").merge();
ruleMaster.getRange("A1").values = [["공통 코어 에이전트 샘플 결과 v2 - Rule Master"]];
ruleMaster.getRange("A1").format = {
  fill: subHeaderFill,
  font: { bold: true },
  horizontalAlignment: "Left",
};
ruleMaster.getRange("A2:R2").values = ruleHeaders;
ruleMaster.getRange(`A3:R${ruleRows.length + 2}`).values = ruleRows;
formatSheet(
  ruleMaster,
  "A2:R2",
  `A3:R${ruleRows.length + 2}`,
  [["A", 12], ["B", 10], ["C", 16], ["D", 12], ["E", 12], ["F", 14], ["G", 14], ["H", 14], ["I", 14], ["J", 14], ["K", 14], ["L", 10], ["M", 26], ["N", 16], ["O", 12], ["P", 20], ["Q", 30], ["R", 10]],
);
ruleMaster.getRange(`Q3:Q${ruleRows.length + 2}`).format.fill = draftFill;
ruleMaster.getRange(`R3:R${ruleRows.length + 2}`).format.fill = draftFill;

noteMaster.getRange("A1:H1").merge();
noteMaster.getRange("A1").values = [["공통 코어 에이전트 샘플 결과 v2 - Note Master"]];
...
formatSheet(
  ...
  [["A", 12], ["B", 20], ["C", 42], ["D", 16], ["E", 18], ["F", 10], ["G", 18], ["H", 30]],
);
noteMaster.getRange(`H3:H${noteRows.length + 2}`).format.fill = draftFill;

ruleNoteMap.getRange("A1:F1").merge();
ruleNoteMap.getRange("A1").values = [["공통 코어 에이전트 샘플 결과 v2 - Rule-Note Map"]];
...
changeLog.getRange("A1").values = [["공통 코어 에이전트 샘플 결과 v2 - Change Log"]];
...
formatSheet(
  ...
  [["A", 12], ["B", 10], ["C", 14], ["D", 28], ["E", 28], ["F", 18], ["G", 12], ["H", 26]],
);
